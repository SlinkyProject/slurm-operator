...
func WaitForCommand(ctx context.Context, t *testing.T, command string, args []string, wants string, cleanupCommand string, cleanupArgs []string, timeout time.Duration, retryDelay time.Duration) {
	...
	err := wait.For(func(waitCtx context.Context) (bool, error) {
		...
		return commandErr == nil && commandOutputMatches(output, wants), nil
	},
...
// commandOutputMatches accepts a repeated expected value because older Slurm
// configurations can report the same node once per partition.
func commandOutputMatches(output []byte, wants string) bool {
	if wants == "" {
		return true
	}
