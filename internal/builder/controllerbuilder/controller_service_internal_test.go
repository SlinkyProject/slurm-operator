...
func TestBuilder_BuildControllerServiceInternal(t *testing.T) {
	type fields struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := b.BuildControllerServiceInternal(tt.args.controller)
			if err != nil {
				t.Fatalf("Builder.BuildControllerService() error = %v", err)
			}
			if got.Name != "slurm-controller-internal" {
				t.Errorf("name = %q, want slurm-controller-internal", got.Name)
			}
