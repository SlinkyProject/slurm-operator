...
// specify one. Marking the Slurm node down and deleting its record as the pod
// terminates stops the Slurm controller from assigning work to a node that is
// going away.
//
	...
	return &corev1.LifecycleHandler{
		Exec: &corev1.ExecAction{
			Command: []string{
				"/usr/bin/sh",
				"-c",
				"scontrol update nodename=$(hostname) state=down reason='slurm-operator: Pod is terminating' && scontrol delete nodename=$(hostname);",
			},
		},
	}
}
