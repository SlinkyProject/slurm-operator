...
| epilogScriptRefs | list | `[]` | References to existing ConfigMaps containing Slurm Epilog scripts to mount in `/etc/slurm`. Each entry is an object with a `name` field (the ConfigMap name); additive to `epilogScripts`. The ConfigMaps must already exist in the release namespace. Keys (filenames) become `Epilog=` values, so follow the `epilog-*` naming used by `epilogScripts`. WARNING: Keys must be unique across this list, `epilogScripts`, any enabled vendor scripts (e.g. DCGM), and every other ConfigMap mounted in `/etc/slurm`. Duplicates **fail** the controller pod at projected-volume mount time. |
...
| epilogSlurmctldScriptRefs | list | `[]` | References to existing ConfigMaps containing Slurm EpilogSlurmctld scripts to mount in `/etc/slurm`. Each entry is an object with a `name` field (the ConfigMap name); additive to `epilogSlurmctldScripts`. The ConfigMaps must already exist in the release namespace. Each key (filename) becomes an `EpilogSlurmctld=/etc/slurm/<filename>` line, so follow the `epilog-slurmctld-*` naming used by `epilogSlurmctldScripts`. WARNING: Keys must be unique across this list, `epilogSlurmctldScripts`, and every other ConfigMap mounted in `/etc/slurm`. Duplicates **fail** the controller pod at projected-volume mount time. |
...
| prologScriptRefs | list | `[]` | References to existing ConfigMaps containing Slurm Prolog scripts to mount in `/etc/slurm`. Each entry is an object with a `name` field (the ConfigMap name); additive to `prologScripts`. The ConfigMaps must already exist in the release namespace. Keys (filenames) become `Prolog=` values, so follow the `prolog-*` naming used by `prologScripts`. WARNING: Keys must be unique across this list, `prologScripts`, any enabled vendor scripts (e.g. DCGM), and every other ConfigMap mounted in `/etc/slurm`. Duplicates **fail** the controller pod at projected-volume mount time. |
...
| prologSlurmctldScriptRefs | list | `[]` | References to existing ConfigMaps containing Slurm PrologSlurmctld scripts to mount in `/etc/slurm`. Each entry is an object with a `name` field (the ConfigMap name); additive to `prologSlurmctldScripts`. The ConfigMaps must already exist in the release namespace. Each key (filename) becomes a `PrologSlurmctld=/etc/slurm/<filename>` line, so follow the `prolog-slurmctld-*` naming used by `prologSlurmctldScripts`. WARNING: Keys must be unique across this list, `prologSlurmctldScripts`, and every other ConfigMap mounted in `/etc/slurm`. Duplicates **fail** the controller pod at projected-volume mount time. |
