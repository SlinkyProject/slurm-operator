...
| nodesetDefaults.preferKubernetesNodeName | bool | `true` | Prefer Node-derived Slurm names for pinned, non-oversubscribed StatefulSets; otherwise use Pod hostnames. Immutable preference, defaults to true; set false to opt out. DaemonSet naming is unchanged. |
...
| nodesetDefaults.slurmd.lifecycle | corev1.Lifecycle | `{}` | The container lifecycle hooks. A `preStop` hook set here replaces the operator default, which marks the Slurm node down as the pod terminates: `scontrol update nodename="$SLURM_NODE_NAME" state=down reason='slurm-operator: Pod is terminating';` Ref: https://kubernetes.io/docs/concepts/containers/container-lifecycle-hooks/ |
