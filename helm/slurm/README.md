...
| accounting.extraConf | string | `""` |  Raw Slurm accounting configuration content to append to slurmdbd.conf. This allows direct specification of configuration in native Slurm format.  When specified, completely overrides extraConfMap (no merging). Use this for: configuration management systems, direct copy-paste from existing configs. Example:   extraConf: |     CommitDelay=1     PurgeJobAfter=12month     PurgeEventAfter=12month     DebugLevel=debug2 |
| accounting.extraConfMap | map[string]string | map[string][]string | `{}` |  Structured Slurm accounting configuration as key-value pairs to append to slurmdbd.conf.  Only used when extraConf is empty (string format takes complete precedence). Use this for: simple key-value configs, Helm templating, dynamic configuration. Values can be strings or arrays (arrays will be comma-separated). Example:   extraConfMap:     CommitDelay: "1"     PurgeJobAfter: "12month"     DebugFlags: ["DB_EVENT", "DB_JOB"] |
...
| controller.extraConf | string | `""` |  Raw Slurm configuration content to append to slurm.conf. This allows direct specification of configuration in native Slurm format.  When specified, completely overrides extraConfMap (no merging). Use this for: configuration management systems, complex node definitions, direct copy-paste from existing configs. Leverages Slurm's native top-to-bottom precedence - can override any template default. Example:   extraConf: |     ReturnToService=2     MaxNodeCount=1024     AuthType=auth/munge     NodeName=gpu[001-100] NodeAddr=gpu[001-100] CPUs=40 RealMemory=128000 Gres=gpu:tesla:4 |
| controller.extraConfMap | map[string]string | map[string][]string | `{}` |  Structured Slurm configuration as key-value pairs to append to slurm.conf.  Only used when extraConf is empty (string format takes complete precedence). Use this for: simple key-value configs, Helm templating, dynamic configuration. Values can be strings or arrays (arrays will be comma-separated). Example:   extraConfMap:     ReturnToService: "2"     MaxNodeCount: "1024"     GresTypes: ["gpu", "network"] |
