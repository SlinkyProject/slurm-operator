...
| loginsetDefaults | object | `{"enabled":true,"extraSshdConfig":"ClientAliveInterval 60\nClientAliveCountMax 3","initconf":{"image":{"digest":null,"repository":"docker.io/library/alpine","tag":"latest"},"resources":{}},"login":{"env":[],"image":{"digest":null,"repository":"ghcr.io/slinkyproject/login","tag":"26.05-ubuntu26.04"},"resources":{},"securityContext":{"privileged":false},"volumeMounts":[]},"metadata":{},"podSpec":{"affinity":{},"initContainers":[],"nodeSelector":{"kubernetes.io/os":"linux"},"resources":{},"tolerations":[],"volumes":[]},"replicas":1,"rootSshAuthorizedKeys":null,"service":{"metadata":{},"spec":{"type":"LoadBalancer"}},"strategy":{}}` | Defines defaults for the LoginSet map values. |
| loginsetDefaults.enabled | bool | `true` | Enable use of this LoginSet. |
| loginsetDefaults.extraSshdConfig | string | `"ClientAliveInterval 60\nClientAliveCountMax 3"` | Extra configuration lines appended to `/etc/ssh/sshd_config`. The default sends keepalives so that idle sessions survive the idle timeout of the load balancer fronting the login service, and so that sessions whose client went away are reaped instead of lingering. Ref: https://manpages.ubuntu.com/manpages/resolute/man5/sshd_config.5.html |
...
| nodesetDefaults.ssh.extraSshdConfig | string | `"ClientAliveInterval 60\nClientAliveCountMax 3"` | Extra configuration lines appended to `/etc/ssh/sshd_config`. The default sends keepalives so that sessions whose client went away are reaped instead of lingering and holding the job they were adopted into. Ref: https://manpages.ubuntu.com/manpages/resolute/man5/sshd_config.5.html |
