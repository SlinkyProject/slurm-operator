...
| namespace.hook.image | string | object | `{"repository":"registry.k8s.io/kubectl","tag":"v1.30.2"}` | The image to use for the namespace labeler hook job. |
| namespace.labels.enabled | bool | `true` | Labels the namespace with `slinky.slurm.net/cluster` when true. Required for the pod binding webhook namespaceSelector. |
