...
| restapi.podDisruptionBudget | object | `{"enabled":true,"minAvailable":1}` | PodDisruptionBudget for the slurmrestd Deployment when `replicas` is greater than one. Uses policy/v1 PodDisruptionBudgetSpec fields (`minAvailable`, `maxUnavailable`, etc.); the operator sets `selector`. Ref: https://kubernetes.io/docs/reference/kubernetes-api/policy-resources/pod-disruption-budget-v1/ |
| restapi.podDisruptionBudget.enabled | bool | `true` | When true (default), the operator reconciles a PDB while `replicas` > 1. Set to false to disable. |
| restapi.podDisruptionBudget.minAvailable | int | `1` | Example: PodDisruptionBudget `spec.minAvailable`. If neither `minAvailable` nor `maxUnavailable` is set, the operator defaults `minAvailable` to 1. |
