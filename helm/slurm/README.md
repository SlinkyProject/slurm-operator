...
| qos | map[string]object | `{}` | Slurm QOS configurations (requires accounting.enabled=true). The map key is the QOS name; the map value contains the QOS definition. Config keys map directly to V0044Qos JSON fields from the Slurm REST API. Ref: https://slurm.schedmd.com/qos.html Ref: https://slurm.schedmd.com/rest.html |
